...
load_haplotypes <- function(population, RIL, chromosomes) {
  ...
  if(! file.exists(haplotypes_file_wide)) {
    ...
    if(RIL == TRUE) {
      ...
      o <- foreach(n=1:max(all_haplotypes[,ind])) %dopar% {
        o2 <- foreach(chr=chromosomes) %do% {
          print(paste(n, chr))
      ...
      o <- foreach(ind_i = 1:10000) %dopar% {
        o2 <- foreach(chr=chromosomes) %do% {
          print(paste(ind_i, chr))
...
get_dosage_at_causative_loci <- function(haps.DT, causative.DT) {
  o2 <- foreach(ind_n=1:max(haps.DT[,ind])) %dopar% {
    # print(ind_n)
    haplotypes <- haps.DT[ind==ind_n]
    oB <- foreach(i=1:nrow(haplotypes), .errorhandling="remove") %do% {
...
calcFreqs <- function(dat.haps, dat.vcf) {
    ...
    # While counting the number of times each haplotype is present in the population
        range.haplotype.counts.long <- rbindlist(foreach(start.i = segments.to.iter[containsSNP==TRUE][,start], stop.i=segments.to.iter[containsSNP==TRUE][,stop]) %do% {
